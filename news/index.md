...
- The copyright holder is now **Folkehelseinstituttet**. It read
  “cs9example authors”, which was a template placeholder.
...
- [`library(cs9example)`](https://www.csids.no/cs9/) now runs with no
  ...
  `file.path(tempdir(), "cs9example")` and calls
...
- `.onLoad()` now calls
  ...
  on the same terms.
- The package became loadable at all once `plnr` was fixed. `.onLoad()`
  calls `set_tasks()`, which reaches
  `plnr::Plan$add_analysis_from_list()`, and that method read an
  undefined `df` that only resolved because
  [`stats::df`](https://rdrr.io/r/stats/Fdist.html) happened to be on
  the search path. Loading died there first, which is why the
  [`assignInNamespace()`](https://rdrr.io/r/utils/getFromNamespace.html)
...
- `DESCRIPTION` now declares `Config/Needs/website: niphr/cstemplate`,
  ...
  declare the same value; plnr declares `raubreywhite/pkgdowntemplate`.
- `DESCRIPTION` `Imports:` now matches what the code actually uses. Nine
  namespaces were reached through `::` without being declared, and are
  now declared: `csdb`, `csmaps`, `cstidy`, `cstime`, `glue`, `httr`,
  `plnr`, `progressr` and `stringr`. Three were declared without being
  used anywhere in the package, and are removed: `janitor`, `merTools`
  and `tidyr`. None of the three appears in any file outside
  `DESCRIPTION` itself, including string literals; `tidyr` was
  superseded by
  [`data.table::CJ()`](https://rdrr.io/pkg/data.table/man/J.html) in
...
- `rmarkdown` stays in `Imports:` even though `R CMD check` reports it
  as unused. `.onLoad()` calls
  `utils::assignInNamespace("clean_tmpfiles", clean_tmpfiles_mod, ns = "rmarkdown")`,
  which names the package as a string, so no static analysis can see it.
  Loading cs9example does load the `rmarkdown` namespace, and
  [`assignInNamespace()`](https://rdrr.io/r/utils/getFromNamespace.html)
...
- Added a pkgdown site: `_pkgdown.yml` on the `cstemplate` house
  template, an `index.md` home page, and a generated hex logo
  (`dev/logo.R` -\> `man/figures/logo.png`).
- Documented every exported function with a description, `@seealso` and
  `@family`, and added runnable examples where one can run without a
  database.
...
- [`make_skeleton_date()`](https://niphr.github.io/cs9example/reference/make_skeleton_date.md)
  and
  [`make_skeleton_isoyearweek()`](https://niphr.github.io/cs9example/reference/make_skeleton_isoyearweek.md)
  are now actually exported. Both carried `@export` since they were
  added, but `NAMESPACE` had not been regenerated, so neither was
  reachable as
  [`cs9example::make_skeleton_date()`](https://niphr.github.io/cs9example/reference/make_skeleton_date.md).
...
- Updated database schema to include quarterly fields (isoquarter,
  isoyearquarter)
...
- Streamlined package structure by removing unused COVID-19
  functionality
